...
rule("Locations without the right number of locationPolice are crime ridden", ({locationPopulation, locationPolice}) => ({
  $location: {
    locationPopulation,
    locationPolice,
  }
  ...
  then: ({$location: {id, locationPopulation, locationPolice}}) => {
    const locationIsCrimeRidden = locationPolice/locationPopulation < CRIME_RATIO
    insert({
      [id]: {
...
rule("Locations with too few locationFarmers need food", ({locationPopulation, locationFarmers}) => ({
  $location: {
    locationPopulation,
    locationFarmers
  }
...
rule("Unincorporated territory has a small locationPopulation", ({locationPopulation}) => ({
  $location: {
    locationPopulation
  }
})).enact({
  when: ({$location: { locationPopulation}}) => locationPopulation < 10,
  then: ({$location: {id}}) => insert({[id]: {locationClassification: LocationClassification.UNINCORPORATED}})
})

rule("Villages have a sizeable locationPopulation", ({locationPopulation}) => ({
  $location: {
    locationPopulation
  }
})).enact({
  when: ({$location: { locationPopulation}}) => locationPopulation > 10 && locationPopulation < 1000,
  then: ({$location: {id}}) => insert({[id]: {locationClassification: LocationClassification.VILLAGE}})
})
...
rule("Cities have a large locationPopulation", ({locationPopulation}) => ({
  ...
  then: ({$location: {id}}) => insert({[id]: {locationClassification: LocationClassification.CITY}})
})

rule("Locations locationHappiness is determined by their problems", ({locationIsCrimeRidden, locationIsHungry, locationIsInDisrepair}) => ({
  $location: {
    locationIsCrimeRidden, locationIsHungry, locationIsInDisrepair
  }
})).enact({
  then: ({$location: {id, locationIsHungry, locationIsInDisrepair, locationIsCrimeRidden}}) => {
    const numProblems = [locationIsHungry, locationIsCrimeRidden, locationIsInDisrepair].filter(Boolean).length
...
rule("A locations workforce is the sum of it's civilians", ({civilianLocationId, civilianJob}) => ({
  $civilian: {
    civilianLocationId,
    civilianJob
  }
  ...
  thenFinally: (getCivilians) => {
    ...
    getCivilians().forEach(({$civilian: {civilianLocationId, civilianJob}}) => {
      if(!locToCount.has(civilianLocationId)) locToCount.set(civilianLocationId, new Map<CivilianJob, number>())
      if(!locToCount.get(civilianLocationId)?.has(civilianJob)) locToCount.get(civilianLocationId)?.set(civilianJob, 0)
...
rule("A Locations population is the sum of it's parts", ({locationFarmers, locationCivics, locationWorkers, locationPolice}) => ({
  $location: {
    locationWorkers, locationFarmers, locationCivics, locationPolice
  }
})).enact({
  then:({$location: {id, locationWorkers, locationFarmers, locationPolice, locationCivics}}) => {
    insert({
      [id]: {
        locationPopulation: locationFarmers + locationCivics + locationPolice + locationWorkers
      }
    })
  }
})
...
rule("A Provinces population is the sum of it's parts", ({locationProvinceId, locationPopulation}) => ({
  $location: {
  ...
  thenFinally:(getResults) => {
    ...
    locations.forEach(loc => {
      ...
      provinceToPop.set(loc.$location.locationProvinceId, loc.$location.locationPopulation + curPop)
    })
...
export const provinces = rule("Provincies", ({provinceName, provinceClassification, provincePopulation}) => ({
  $province: {
    provinceName,
    provincePopulation,
...
export const locations = rule("Locations", ({
                                              locationName, locationPopulation, locationFarmers, locationPolice, locationWorkers, locationCivics, locationHappiness, }) => ({
  $location: {
    locationName,
    locationPopulation,
    locationFarmers,
...
export const civilians = rule("Civilians", ({civilianFirstName, civilianLastName, civilianLocationId, civilianJob, }) => ({
  $civilian: {
    civilianFirstName,
    civilianLastName,
    civilianLocationId,
