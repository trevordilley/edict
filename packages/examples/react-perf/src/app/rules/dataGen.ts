...
const newId = (name: string) => name.toLowerCase()
  .replace(/\s+/g, '_')
  .replace(/[^\u0100-\uFFFF\w\-]/g,'-')
  .replace(/\_\_+/g, '_')
...
export const newCivilian = (civilianLocationId: string): {id: string, civilian: Civilian} => {
  const civilianJob = _.shuffle([CivilianJob.WORKER, CivilianJob.CIVIC, CivilianJob.FARMER, CivilianJob.POLICE]).shift()!
...
export const newProvince = (): {id: string, province: Province} => {
  ...
  return {id: newId(provinceName), province: {
...
export const newLocation = (provinceId: string):{ location: { locationFarmers: number; locationName: string; locationWorkers: number; locationPolice: number; locationHappiness: LocationHappiness; locationCivics: number; locationIsHungry: boolean; locationClassification: LocationClassification.UNINCORPORATED; locationIsInDisrepair: boolean; locationIsCrimeRidden: boolean; locationPopulation: number; locationProvinceId: string }; id: string } => {
...
export const newWorld = (numProvinces: number, maxLocations: number, maxPop: number) => {
  ...
  for(let i = 0; i < numProvinces; i++){
    ...
    for(let j = 0; j < numLoc; j++) {
      const {id: locationId, location} = newLocation(provinceId)
