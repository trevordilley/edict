...
// new independent sessions
    ...
    rule('When a birthday is today, celebrate the birthday!',
      ({ birthDay, todaysDate }) =>
        ({
    ...
    const usersCelebratingBirthdays = rule("All users celebrating their birthday", ({ isCelebratingBirthDay, fullName }) =>
      ({
        $user: {
          fullName,
