// import {edict, insertFactToFact, Rules, WHAT_SCHEMA} from "@edict/core";
...
describe('utils', () => {
  ...
  it("inserted fact is converted to internal representation", () => {
    const fact = {npc: {health: 10, x: 12, y: 13}}

    const result = insertFactToFact(fact)

    expect(result).toStrictEqual([["npc", "health", 10],
    ["npc","x",12],
      ["npc","y", 13]
    ])
  })
